...
class ImageList(datasets.VisionDataset):
    ...
    def __init__(self, root, data_list_file, filter_class, transform=None, target_transform=None):
        super().__init__(root, transform=transform, target_transform=target_transform)
        if 'pacs' in root:
            self.target_minus = True # for pacs whose targets start from 1
        else:
            self.target_minus = False # for other datasets whose targets start from 0
        ...
        # count = [sum(np.array(self.label)==i) for i in range(1000)]
        # count, _ = np.histogram(self.label, 10000)
        # print(count)
    ...
    def parse_data_file(self, file_name, filter_class):
        ...
        with open(file_name, "r") as f:
            ...
            for line in f.readlines():
                ...
                if (filter_class is not None) and (target not in filter_class):
                    continue
